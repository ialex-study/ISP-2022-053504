...
class ComplexSerializer():
    ...
    def _serialize_function(self, function: FunctionType) -> dict:
        result = {
        ...
        function_closure = function.__closure__

        result["__code__"].update(self._serialize_code(function_code))
        
        for key, value in function_globals.items():
            ...
            if key == function.__name__:
                global_value = function.__name__
            
            elif inspect.isclass(value):
                global_value = "CLASS"
            
            else:
                global_value = self._serialize_object(value)

            if global_value is not None:
                result["__globals__"][key] = global_value
        ...
        if function_closure is not None:
            for cell in function_closure:
                ...
                if inspect.isclass(cell_content):
                    ...
                    result["__closure__"].append(self._serialize_object(cell_content))
    ...
    def _serialize_class(self, _class: type) -> dict:
        ...
        if _class == type:
            ...
            result["__bases__"] = [self._serialize_class(base) for base in _class.__bases__ if base != object]
            ...
            for member in inspect.getmembers(_class):
                if member[0] not in (
                    ...
                    result["members"][member[0]] = self._serialize_object(member[1])
    ...
    def _serialize_module(self, module: ModuleType) -> dict:
        ...
        for key, value in inspect.getmembers(module):
            ...
            result["members"][key] = self._serialize_object(value)
    ...
    def _serialize_instance(self, instance: Any) -> dict:
        ...
        for key, value in inspect.getmembers(instance):
            if key.startswith("__") or inspect.isroutine(value):
                continue
    ...
    def _deserialize_code(self, code_dict: dict) -> CodeType:
        ...
        for key, value in code_dict.items():
            function_code[key] = self._deserialize_object(value)
    ...
    def _deserialize_function(self, function_dict: dict, _class = None) -> FunctionType:
        ...
        function_globals = {}
        for key, value in function_dict["__globals__"].items():
            function_globals[key] = self._deserialize_object(value)
    ...
    def _deserialize_module(self, module_dict: dict) -> ModuleType:
        ...
        for key, value in module_dict["members"].items():
            setattr(module, key, self._deserialize_object(value))
        
        return module
    
    def _deserialize_class(self, class_dict: dict) -> type:
        class_name = class_dict["__name__"]
        class_bases = tuple(self._deserialize_class(base) for base in class_dict["__bases__"])
        class_methods = {}
        ...
        for method_name, method_value in class_methods.items():
            setattr(_class, method_name, self._deserialize_function(method_value, _class=_class))
    ...
    def _deserialize_instance(self, instance_dict: dict) -> Any:
        ...
        instance_members = {}
        for key, value in instance_dict["members"].items():
            instance_members[key] = self._deserialize_object(value)
